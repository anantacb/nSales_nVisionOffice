...
import TableIndicesFormComponent from "@/views/database/ManageTableIndices/TableIndicesForm.vue";
...
const tableIndicesRef = ref(null);

function addNewIndex() {
    tableFieldsForm.value.addNewIndex();
}
...
function startLoading() {
    tableIndicesRef.value.statusLoading();
}

function endLoading() {
    tableIndicesRef.value.statusNormal();
}

async function saveWithoutExecuting() {
    templateStore.pageLoader({mode: 'on'});
    try {
        let {data, message} = await TableIndex.tableIndicesOperationsSaveWithoutExecuting(
            table.value.Id,
            sqlFormData.value
        );
...
async function saveAndExecute() {
    templateStore.pageLoader({mode: 'on'});
    try {
        let {data, message} = await TableIndex.tableIndicesOperationsSaveAndExecute(table.value.Id, sqlFormData.value);
        modal.value.closeModal();
        notificationStore.showNotification(message);
        formComponentKey.value += 1;
    } catch (error) {
...
<template>
    ...
    <div class="content">
        <BaseBlock
            ref="tableIndicesRef"
            :title="`${table.Name} Table Indices`"
        ...
        >
            <template #options>
                ...
                <button class="btn btn-sm btn-outline-primary me-1" type="button"
                        @click="addNewIndex">Add Field
                </button>
            </template>

            <TableIndicesFormComponent :key="formComponentKey"
                                       ref="tableFieldsForm"
