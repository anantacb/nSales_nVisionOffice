...
const ApplyToOptions = ['', 'Company', 'Role', 'User'].map((item) => {
    return {
...
async function applyToChanged() {
    ...
    switch (ApplyTo.value) {
        case 'Company':
            ...
            makeApplyToDropdownsVisible(['Module', 'Table']);
            break;
        case 'Role':
            await getRolesByCompany();
            makeApplyToDropdownsVisible(['Role', 'Module', 'Table']);
            break;
        case 'User':
            await getAllCompanyUsers();
            makeApplyToDropdownsVisible(['User', 'Module', 'Table']);
            break;
        default:
            break;
    }
...
async function getModulesByCompany() {
    const {data} = await Module.getActivatedModulesByCompany(companyStore.selectedCompany.Id);
...
async function getRolesByCompany() {
    const {data} = await Role.getRolesByCompany(companyStore.selectedCompany.Id);
...
async function createDataFilter() {
    ...
    try {
        let {data, message} = await DataFilter.createCompanyDataFilter(formData);
        createDataFilterRef.value.statusNormal();
        await router.push({name: 'company-data-filters'});
...
<template>
    <div class="content">
        ...
        <BaseBlock ref="createDataFilterRef" :title="`Create Data Filter for (${companyStore.selectedCompany.Name})`" content-full>

            <template #options>
                <router-link :to="{name:'company-data-filters'}" class="btn btn-sm btn-outline-info">
